...
if __name__ == "__main__":
    ...
    high_corr_vars = (
        ...
        .sort_values()
        .loc[lambda x: abs(x) >= CORR_THRESHOLD]
    )
    ...
    imbalances = X[categorical_vars.keys()].apply(
        lambda x: x.value_counts(normalize=True).std()
        / x.value_counts(normalize=True).mean()
    )
    ...
    plot_hist_features(
        X=X[high_cv_vars.index], title="High coefficient of variation variable"
    )
    plot_hist_features(
        X=X[high_corr_vars.index], title="High correlation to target variable"
    )
